export const setupSocketRoutes = (socket, scene) => {
    ...
    socket.on("command.player.GetPlayerInfo", () => {
        ...
        console.log(`Fetching player info for: ${player_name}`);

        if (player_persona) {
            socket.emit("command.player.GetPlayerInfo", JSON.stringify(player_persona.toJSON()));
        } else {
            console.error(`Player persona for ${player_name} not found.`);
        }
    });
    ...
    socket.on("command.npc.GetNPCInfo", (npc_name) => {
        console.log(`Fetching info for NPC: ${npc_name}`);
        ...
        if (npc_info) {
            ...
            console.error(`NPC ${npc_name} not found.`);
        }
    });

    socket.on("command.map.NPCNavigate", (payload) => {
        try {
            console.log("Processing NPC navigate command.");
    ...
    socket.on("command.map.GetMapTown", () => {
        console.log("Fetching map town data.");
    ...
    socket.on("command.chat.NPCChatUpdate", (payload) => {
        try {
            console.log("Processing NPC chat update.");
